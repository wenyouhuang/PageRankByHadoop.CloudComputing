...
public class PageRankDriver {
	...
	public static final String CACHE_DIR = "s3://wenyouhuang-pagerank-cs549/cache"; // for EMR
	...
	static void init(String input, String output, int reducers)
		...
		/*
		 * init inputDir outputDir #reducers: This job read the file(s) in the input directory, 
		 * convert thems into your intermediate format, and outputs the data to the output directory, 
		 * using the specified number of reducers.
		 */
	...
	static void iter(String input, String output, int reducers)
			throws IOException, ClassNotFoundException, InterruptedException {
		/*
		 * iter inputDir outputDir #reducers: This job performs a single round of PageRank by 
		 * reading data in your intermediate format from the input directory, and writing data 
		 * in your intermediate format to the output directory, using the specified number of reducers.
		 */
	...
	static void diff(String input1, String input2, String output, int reducers)
			throws Exception {
		/*
		 * diff inputDir1 inputDir2 outputDir #reducers: This job reads data in your intermediate format from 
		 * both input directories and outputs a single line that contains the maximum difference between any pair 
		 * of rank values for the same vertex. You must use absolute values for the differences, i.e., the change 
		 * from 0.98 to 0.97 is 0.01. While diff runs as a single task, it is implemented using two different MapReduce 
		 * jobs run successively.
		 */
	...
	static void finish(String input, String output, int reducers)
			throws Exception {
		/*
		 * finish inputDir outputDir #reducers: This job reads data in your intermediate format from 
		 * the input directory, converts the data to the output format, and outputs it to the output 
		 * directory, using the specified number of reducers.
		 */
	...
	static void finishjoin(String input, String output, int reducers)throws Exception{
		...
		FileInputFormat.addInputPath(job,new Path(input));
		FileInputFormat.addInputPath(job, new Path(PageRankDriver.CACHE_DIR));
	...
	static void finishCombine(String input, String output, int reducers)throws Exception{
	...
	public static void composite(String input, String output, String interim1,
		...
		/*
		 * composite inputDir outputDir intermDir1 intermDir2 diffDir #reducers: This function runs the 
		 * init task, then it alternates between running the iter and diff tasks until convergence has occurred 
		 * (diff <=30 in the case of the test data), at which point it runs the finish task and places the output 
		 * into <outputDir>. Note, running the diff task after every iteration could add considerable time to your 
		 * job, so this function runs the diff task after every two or three iterations to save computation time.
		 */
		/*
		 * TODO 
		 */
		System.out.println("WenyouHuang (10412235)");
		...
		while (difference >= PageRankDriver.MAX_DIFFERENCE) // continues operation till difference is of specified value
		{
			if (i % 2 == 0) {
